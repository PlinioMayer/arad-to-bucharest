...
def buscar_menor_caminho(caminho_atual: Caminho, caminhos: List[Caminho] = [], melhor_caminho: Caminho = None):
    ...
    for caminho_index, caminho in enumerate(caminhos):
        ...
        if (caminho.distancia + caminho.cidades[-1].distancia_bucharest) < \
                (menor_caminho.distancia + caminho.cidades[-1].distancia_bucharest):
            menor_caminho = caminho
            menor_caminho_index = caminho_index
    ...
    for rota in caminho_atual.cidades[-1].rotas:
        ...
        if novo_melhor_caminho and \
                (novo_caminho.distancia + novo_caminho.cidades[-1].distancia_bucharest) > novo_melhor_caminho.distancia:
            adicionar = False
        else:
            for caminho in caminhos:
                if novo_caminho.cidades[-1] is caminho.cidades[-1] and \
                        (novo_caminho.distancia + novo_caminho.cidades[-1].distancia_bucharest) > \
                        (caminho.distancia + caminho.cidades[-1].distancia_bucharest):
                    adicionar = False

        if adicionar:
            caminhos.append(novo_caminho)
            if menor_caminho:
                if (novo_caminho.distancia + novo_caminho.cidades[-1].distancia_bucharest) < \
                        (menor_caminho.distancia + menor_caminho.cidades[-1].distancia_bucharest):
